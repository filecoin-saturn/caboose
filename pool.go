...
func (p *pool) fetchResourceWith(ctx context.Context, path string, cb DataCallback, with string) (err error) {
	...
	if seen502 {
		failedCarFetchRetriesTotalMetric.WithLabelValues("502", fmt.Sprintf("%d", attempt)).Add(1)
	}

	if seen504 {
		failedCarFetchRetriesTotalMetric.WithLabelValues("504", fmt.Sprintf("%d", attempt)).Add(1)
	}

	if seenPartial {
		failedCarFetchRetriesTotalMetric.WithLabelValues("partial", fmt.Sprintf("%d", attempt)).Add(1)
	}
