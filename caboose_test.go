...
func TestCidCoolDown(t *testing.T) {
	...
	ch := util.BuildCabooseHarness(t, 3, 3, util.WithMaxFailuresBeforeCoolDown(2), util.WithCidCoolDownDuration(1*time.Second))
	...
	ch.RecoverNodes(t, func(e *util.Endpoint) bool {
		return true
	})
...
func TestFetchBlock(t *testing.T) {
	ctx := context.Background()
	h := util.BuildCabooseHarness(t, 3, 3)
	...
	// all nodes fail
	h.FailNodesWithCode(t, func(e *util.Endpoint) bool {
		return true
...
func TestResource(t *testing.T) {
	h := util.BuildCabooseHarness(t, 1, 3)
	// some setup.
	...
	h.Endpoints[0].Resp = buf.Bytes()
	h.Endpoints[0].CarWrap = false

	// ask for it.
	if err := h.Caboose.Fetch(context.Background(), "/path/to/car", func(resource string, reader io.Reader) error {
	...
	second := false
	if err := h.Caboose.Fetch(context.Background(), "/path/to/car1", func(resource string, reader io.Reader) error {
